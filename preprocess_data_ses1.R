...
for (k in 1:length(subjects)) {
  ...
  bold_files <- list.files(
    path = paste0(base_dir, "/sub-", sub, "/ses-1/func/"),
  ...
  # ---- Timing detection ----
  timing_path <- paste0(timing_dir, "/", sub, "/Timing_Files_Pre/")
  ...
  for (i in 1:2) {
    ...
    for (m in valid_runs) {
      bold_path <- paste0(base_dir, "/sub-", sub, "/ses-1/func/sub-", sub,
                          "_ses-1_task-GNG_run-", m, "_masked.nii.gz")
      conf_path <- paste0(base_dir, "/sub-", sub, "/ses-1/func/sub-", sub,
                          "_ses-1_task-GNG_acq-epi_run-", m,
  ...
  for (q in valid_runs) {
    ...
    timing_prefix <- paste0(timing_dir, "/", sub, "/Timing_Files_Pre/sub-", sub, "_GoNoGo_")
  ...
  png(paste0(fig_dir, "/sub-", sub, "_ses1.png"), 
  ...
  mtext(paste0("Subject ", sub, " — Session 1"), line = 1, outer = TRUE, cex = 1.3)
  
  dev.off()
  cat("Saved figure for subject:", sub, "\n")
  
  save(dat, file = paste0(data_dir, "/sub-", sub, "_ses1.RData"))
