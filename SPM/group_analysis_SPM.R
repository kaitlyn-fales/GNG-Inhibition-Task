...
for (i in 1:nrow(df)){
  ...
  DCM <- readMat(paste0("../Output/DCM_sub_", df$subject[i],
                        "_ses1_run", df$run[i], "_", df$region[i],"_", df$VOI_type[i], "_roi_out.mat"))
  
  # Posterior means
  means <- c(DCM$Ep.A,DCM$Ep.B,DCM$Ep.C)
  mu <- means[c(1:2,4,9:10,13)]
  ...
  # Rows/cols same for all subjects
  Cp <- Cp[-c(3,5:8,11:12,14:16),-c(3,5:8,11:12,14:16)]
  ...
  draws[,idx_B] <- -0.5*draws[,idx_A[1]]*exp(draws[,idx_B-1])
